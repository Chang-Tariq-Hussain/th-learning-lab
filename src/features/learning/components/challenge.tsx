...
export function Challenge({ scenario, colorToken, experiment, onVerify, alreadySolved, onAttempt }: ChallengeProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* The live simulation is a plain sibling here, not a child of
          the card above — full width, bounded only by whatever
          contains this `Challenge`, exactly like Explore's
          presentation, rather than boxed in by this scenario's own
          card padding. */}
      {showExperiment ? (
        <ExperimentFrame label="Use the simulation to work this out" colorTextClassName={colors.text}>
          {experiment}
        </ExperimentFrame>
      ) : null}
