...
interface PeriodChallengeProps {
  step: number;
...
export function PeriodChallenge({ step, feedback, onAnswer, onRestart }: PeriodChallengeProps) {
  ...
  return (
    <div className="rounded-card border border-line bg-white/60 p-4 dark:border-line-dark dark:bg-white/[0.03]">
      <div className="flex items-center justify-between">
        <p className="font-display text-base font-medium text-ink dark:text-bone">Period Challenge</p>
        {done ? (
          <button
            type="button"
            onClick={onRestart}
      ...
      {done ? (
        <p className="mt-2 text-sm font-medium text-pine-600 dark:text-pine-300">
          ✓ Nice work — frequency and period make sense now.
        </p>
      ) : (
        <>
