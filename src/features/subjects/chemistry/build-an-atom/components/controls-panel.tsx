...
interface ControlsPanelProps {
  onChange: (particle: "proton" | "neutron" | "electron", delta: 1 | -1) => void;
  protons: number;
  neutrons: number;
  electrons: number;
}

const rows: { key: "proton" | "neutron" | "electron"; label: string; color: string }[] = [
...
export function ControlsPanel({ onChange, protons, neutrons, electrons }: ControlsPanelProps) {
  const counts = { proton: protons, neutron: neutrons, electron: electrons };

  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      {rows.map((row) => (
        <div key={row.key} className="flex flex-col gap-2 rounded-card border border-line bg-white/60 p-4 dark:border-line-dark dark:bg-white/[0.03]">
          <div className="flex items-center gap-2">
            <span aria-hidden="true" className="h-3 w-3 rounded-full" style={{ backgroundColor: row.color }} />
            <span className="font-display text-sm font-medium text-ink dark:text-bone">{row.label}s</span>
            <span className="ml-auto font-mono text-sm text-ink-soft dark:text-bone-soft">{counts[row.key]}</span>
          </div>
