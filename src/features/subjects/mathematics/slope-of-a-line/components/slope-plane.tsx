...
export type Highlight = "none" | "run" | "rise" | "slope";

interface SlopePlaneProps {
  a: SlopePoint;
  b: SlopePoint;
  ...
  highlight?: Highlight;
  draggable?: boolean;
  /** When false, the line is drawn faint/dashed and labeled "m = ?" — used by the Calculate challenge so the answer isn't given away. */
  revealSlope?: boolean;
}
...
/**
 * The corner of the right triangle sits at (b.x, a.y) — level with A
 * horizontally, at B's x. Horizontal leg: A -> corner is the run.
 * Vertical leg: corner -> B is the rise. When run is 0 (a vertical
 * line) the corner collapses onto A, which correctly draws no run leg.
 */
export function SlopePlane({ a, b, onChangeA, onChangeB, highlight = "none", draggable = true, revealSlope = true }: SlopePlaneProps) {
  ...
  const cornerSize = 10;
  const cornerX = runVal >= 0 ? cpx - cornerSize : cpx + cornerSize;
  const cornerY = riseVal >= 0 ? cpy - cornerSize : cpy + cornerSize;
  const showCorner = runVal !== 0 && riseVal !== 0;
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="An interactive coordinate plane showing a line between two points with its rise, run, and slope"
    >
      {/* Grid lines */}
      <g>
        {AXIS_TICKS.map((n) => (
      ...
      {/* Right triangle legs: run (horizontal), rise (vertical) */}
      <g className="pointer-events-none">
        <line
          ...
          x2={cpx}
          y2={cpy}
          strokeWidth={highlight === "run" ? 4 : 2.5}
          strokeDasharray={highlight === "run" ? undefined : "6 4"}
          className={cn("transition-all duration-200", highlight === "run" ? "stroke-pine-500 dark:stroke-pine-300" : "stroke-pine-500/50 dark:stroke-pine-300/50")}
        />
        <line
          x1={cpx}
          y1={cpy}
          x2={bpx}
          y2={bpy}
          strokeWidth={highlight === "rise" ? 4 : 2.5}
          strokeDasharray={highlight === "rise" ? undefined : "6 4"}
          className={cn("transition-all duration-200", highlight === "rise" ? "stroke-amber-500 dark:stroke-amber-400" : "stroke-amber-500/50 dark:stroke-amber-400/50")}
        />
        ...
        <text
          x={(apx + cpx) / 2}
          y={apy + (riseVal >= 0 ? 20 : -12)}
          textAnchor="middle"
          className={cn("font-mono text-[12px] font-semibold", highlight === "run" ? "fill-pine-600 dark:fill-pine-300" : "fill-pine-600/60 dark:fill-pine-300/60")}
        >
          Run = {runVal}
        </text>
        <text
          x={cpx + (runVal >= 0 ? 12 : -12)}
          y={(cpy + bpy) / 2}
          textAnchor={runVal >= 0 ? "start" : "end"}
          className={cn("font-mono text-[12px] font-semibold", highlight === "rise" ? "fill-amber-600 dark:fill-amber-400" : "fill-amber-600/60 dark:fill-amber-400/60")}
        >
          Rise = {riseVal}
        </text>
      </g>

      {/* The line itself (A to B) */}
      <line
        x1={apx}
        y1={apy}
        x2={bpx}
        y2={bpy}
        strokeWidth={highlight === "slope" ? 4.5 : 3}
        strokeDasharray={revealSlope ? undefined : "5 5"}
        className={cn(
          "transition-all duration-200",
          !revealSlope ? "stroke-ink/40 dark:stroke-bone/40" : highlight === "slope" ? "stroke-ink dark:stroke-bone" : "stroke-ink/70 dark:stroke-bone/70"
        )}
      />
      <text
        x={(apx + bpx) / 2 + (riseVal >= 0 ? 14 : -14)}
        y={(apy + bpy) / 2}
        textAnchor={riseVal >= 0 ? "start" : "end"}
        className={cn(
          "pointer-events-none font-mono text-[13px] font-semibold",
          !revealSlope ? "fill-ink/50 dark:fill-bone/50" : highlight === "slope" ? "fill-ink dark:fill-bone" : "fill-ink/70 dark:fill-bone/70"
        )}
      >
        m = {revealSlope ? formatSlope(m) : "?"}
      </text>
