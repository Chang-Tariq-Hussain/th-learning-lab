  ...
  return (
    <g>
      <defs>
        ...
        <filter id="cell-drop-shadow" x="-30%" y="-30%" width="160%" height="160%">
          <feDropShadow dx="0" dy="10" stdDeviation="10" floodColor="#7A3B2C" floodOpacity="0.22" />
        </filter>
      </defs>

      {/* Membrane + cytoplasm fill, with the drop shadow applied once to the whole body */}
      <path d={MEMBRANE_PATH} fill="url(#cytoplasm-fill)" filter="url(#cell-drop-shadow)" />
      ...
      {/* Membrane outline */}
      <path d={MEMBRANE_PATH} fill="none" stroke="#C77A64" strokeWidth={3.5} strokeLinejoin="round" />

      {/* Inner highlight line, suggesting the membrane's bilayer thickness */}
      <path
      ...
      <ellipse cx={148} cy={118} rx={62} ry={40} fill="#FFFFFF" opacity={0.28} />
    </g>
  );
}
